...
public class DigestURL extends MultiProtocolURL implements Serializable {
    ...
    private final byte[] urlHashComputation() {
        ...
        if (p <= 0) {
            ...
            //dom = dom.substring(p + 1); //not necessary to calculate this substring, just the length which is used below
            domLength = dom.length() - (p+1);
        }
        ...
        // return result hash
        final byte[] b = ASCII.getBytes(hashs);
        assert b.length == 12;
        return b;
    }
