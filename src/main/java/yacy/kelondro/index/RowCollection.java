...
import yacy.kelondro.util.ASCII;
import yacy.kelondro.util.UTF8;
...
public class RowCollection implements Sortable<Row.Entry>, Iterable<Row.Entry>, Cloneable, Serializable {
    ...
    final static byte[] _bytes = ASCII.getBytes("__");
    ...
    public synchronized byte[] exportCollection() {
        ...
        entry.setCol(exp_order_type, (this.rowdef.objectOrder == null) ? _bytes : ASCII.getBytes(this.rowdef.objectOrder.signature()));
    ...
    private static StringBuilder randomHash() {
    	return
                new StringBuilder(12).
    		append(Base64Order.enhancedCoder.encodeLongSB(random.nextLong(), 4))
                .append(
    		Base64Order.enhancedCoder.encodeLongSB(random.nextLong(), 4))
                .append(
    		Base64Order.enhancedCoder.encodeLongSB(random.nextLong(), 4));
    }
    ...
    public static void test(final int testsize) throws SpaceExceededException {
    	...
    	for (int i = 0; i < testsize; i++) {
            ...
            int cab = Base64Order.enhancedCoder.compare(a, b);
            int cba = Base64Order.enhancedCoder.compare(b, a);
    	    final int c = cab;
            if (c == 0 && cba != 0)
        ...
        random = new Random(0);
        for (int i = 0; i < testsize / 2; i++) a.add(randomHash().toString().getBytes());
    	...
    	t0 = System.nanoTime();
    	for (int i = 0; i < testsize; i++) {
    		c.add(randomHash().toString().getBytes());
    	}
    	t1 = System.nanoTime();
    	...
    	for (int i = 0; i < testsize; i++) {
    		e.put(r.newEntry(randomHash().toString().getBytes()));
    	}
        ...
        for (int i = 0; i < testsize; i++) {
            final String rh = randomHash().toString();
        ...
        for (int i = 0; i < testsize; i++) {
            if (e.get(randomHash().toString().getBytes(), true) != null) {
                noghosts = false;
                break;
            }
        }
